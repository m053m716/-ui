...
%  map = ui.cubehelix;
%  map = ui.cubehelix(N);
%  map = ui.cubehelix(N,start,rots,sat,gamma);
%  map = ui.cubehelix(N,start,rots,sat,gamma,irange);
%  map = ui.cubehelix(N,start,rots,sat,gamma,irange,domain);
%  map = ui.cubehelix(N,[start,rots,sat,gamma],...)
%  map = ui.cubehelix([],...)
% [map,lo,hi] = ui.cubehelix(...)
%
...
%  ui.cubehelix(3, [0.5,-1.5,1,1], [0.2,0.8]) % irange=[0.2,0.8]
...
%  ui.cubehelix(3, [0.5,-1.5,1,1], [0.2,0.8], [0.3,0.7]) % domain=[0.3,0.7]
...
% load spine
% image(X)
% colormap(ui.cubehelix)
%
%%% New colors for the SURF example:
% [X,Y,Z] = peaks(30);
% surfc(X,Y,Z)
% colormap(ui.cubehelix([],0.7,-0.7,2,1,[0.1,0.9],[0.1,0.9]))
% axis([-3,3,-3,3,-10,5])
%
...
%
% [map,lo,hi] = ui.cubehelix(N, start,rots,sat,gamma, irange, domain)
